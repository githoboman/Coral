            ...
            for task in tasks:
                ...
                keyboard.append([
                    InlineKeyboardButton(
                        " Complete", callback_data=f"complete_{task['id']}"),
                    InlineKeyboardButton(
                        "✏️ Edit", callback_data=f"edit_{task['id']}"),
                    InlineKeyboardButton(
                        "🗑️ Delete", callback_data=f"delete_{task['id']}"),
                    InlineKeyboardButton(
                        " Details", callback_data=f"details_{task['id']}")
                ])
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    logger.info(
        f"button_callback triggered with data={query.data}, user={query.from_user.id}")
    action, task_id = query.data.split('_', 1)
    try:
        if action == "complete":
            supabase.table('tasks').update(
                {'status': 'completed'}).eq('id', task_id).execute()
...
tasks_handler = ConversationHandler(
    ...
    states={
        # Handle callbacks in TASK_LIST state
        'TASK_LIST': [CallbackQueryHandler(button_callback)]
    },
