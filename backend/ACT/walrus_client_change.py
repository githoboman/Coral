...
import base64
import os
from secure_storage import encrypt_data, decrypt_data, encrypt_and_save, load_and_decrypt
...
class WalrusClient:
    ...
    def _encrypt_data_multi_recipient(self, data: Dict[str, Any],
        ...
        try:
            ...
            for public_key_pem in public_keys:
                ...
                encrypted_keys.append(base64.b64encode(encrypted_aes_key).decode('utf-8'))
    ...
    def retrieve_with_support_access(self, blob_id: str,
        ...
        try:
            ...
            # Decrypt
            decrypted_data = self._decrypt_data_hybrid(single_package, private_key)
...
class UserKeyManager(WalrusClient):  # ✅ Inherit from WalrusClient
    ...
    def create_user_keys(self, user_id: str, password: str) -> Tuple[str, bytes]:
        ...
        encrypted_private_key = self.encrypt_private_key(private_key_pem, password)
    ...
    Optional[str]:
        ...
        try:
            ...
            response = requests.put(
                store_url,
                data=encrypted_data_str.encode('utf-8'),  # Convert string to bytes
                headers={'Content-Type': 'application/octet-stream'},
                timeout=30
            )
    ...
    def retrieve_encrypted_object(self, blob_id: str, user_id: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            decrypted_data = decrypt_data(encrypted_data_str, user_id, password)
        ...
        try:
            ...
            if 'encrypted_private_pem' in data and 'public_pem' in data:
                try:
                    ...
                    encrypted_private_pem = base64.b64decode(data['encrypted_private_pem'])
...
class WalrusRegistrationManager:
    ...
    def get_registration_receipt(self, telegram_id: str, password: str, blob_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            receipt = self.key_manager.retrieve_encrypted_object(blob_id, telegram_id, password)
            return receipt
        except Exception as e:
            return None
...
class LocalEncryptedEmailIndex:
    def __init__(self):
        ...
        if not self.index_password:
            raise ValueError("EMAIL_INDEX_PASSWORD not found in environment variables")
...
if __name__ == "__main__":
    ...
    public_key_str, encrypted_private = key_manager.create_user_keys(user_id, user_password)
