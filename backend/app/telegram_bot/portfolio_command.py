...
from app.telegram_bot.utils import load_user_session, save_user_session
...
# Increased retries
async def sui_rpc_call(payload: dict, network: str = 'mainnet', retries: int = 3):
    ...
    raise Exception(
        f"All {network.upper()} RPC endpoints failed after {retries} retries each.")
...
async def get_kiosk_listings(address: str, network: str):
    ...
    try:
        ...
        kiosk_ids = [obj['data']['objectId']
                     for obj in kiosk_objects if obj.get('data')]
        ...
        # Limit to first 5 kiosks to avoid rate limits
        for kiosk_id in kiosk_ids[:5]:
            try:
...
def get_sui_price_usd_cached(_: int = 0) -> float:
    ...
    try:
        response = requests.get(
            "https://api.coingecko.com/api/v3/simple/price?ids=sui&vs_currencies=usd", timeout=10)
        response.raise_for_status()
...
async def can_earn_portfolio_points(user_id: str, context: ContextTypes.DEFAULT_TYPE) -> tuple:
    ...
    try:
        ...
        if password:
            try:
                portfolio_data = load_user_checkin_data(
                    user_id + "_portfolio", password)
                last_portfolio_str = portfolio_data.get('last_portfolio_time')
                if last_portfolio_str:
                    last_portfolio = datetime.fromisoformat(
                        last_portfolio_str.replace('Z', '+00:00'))
                    time_since_last = now_utc - last_portfolio
                    if time_since_last < timedelta(hours=PORTFOLIO_COOLDOWN_HOURS):
                        next_available = last_portfolio + \
                            timedelta(hours=PORTFOLIO_COOLDOWN_HOURS)
...
async def award_portfolio_points(user_id: str, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        session['portfolio_checkins'] = session.get(
            'portfolio_checkins', 0) + 1
        ...
        if password:
            try:
                ...
                portfolio_data = {"portfolio_checkins": [],
                                  "total_portfolio_points": 0}
            ...
            save_user_checkin_data(
                user_id + "_portfolio", password, portfolio_data)
...
async def handle_network_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'network_auto':
        ...
        keyboard = [[InlineKeyboardButton(
            "❌ Cancel", callback_data='cancel_portfolio')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def show_portfolio_page(update: Update, context: ContextTypes.DEFAULT_TYPE, points_awarded: bool = False,
    ...
    try:
        ...
        if full_balances:
            ...
            sorted_balances = sorted(full_balances, key=lambda x: int(
                x['totalBalance']), reverse=True)[:10]
        ...
        if total_staked > 0:
            ...
            if stakes_data:
                validators = ", ".join(
                    {stake['validatorAddress'][:8] + '...' for stake in stakes_data})
        ...
        if context.user_data['current_page'] > 1:
            nav_buttons.append(InlineKeyboardButton(
                "⬅️ Previous", callback_data='previous_page'))
        if new_cursor:
            nav_buttons.append(InlineKeyboardButton(
                "Next ➡️", callback_data='load_more_objects'))
        if nav_buttons:
        ...
        keyboard.append([InlineKeyboardButton(
            "🔄 Refresh", callback_data='refresh_portfolio')])
...
portfolio_handler = ConversationHandler(
    ...
    states={
        ENTER_ADDRESS: [
            CallbackQueryHandler(handle_network_selection,
                                 pattern='^network_'),
            CallbackQueryHandler(handle_cancel_callback,
                                 pattern='^cancel_portfolio$'),
        ...
        SHOW_PORTFOLIO: [
            ...
            CallbackQueryHandler(
                refresh_portfolio, pattern='^refresh_portfolio$'),
        ],
    },
    fallbacks=[
        CommandHandler('cancel', cancel_portfolio),
