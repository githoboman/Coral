...
def get_db_connection():
    ...
    if _connection is None:
        ...
        # THIS IS THE MAGIC
        _connection.execute(f"PRAGMA key = '{DB_ENCRYPTION_KEY}'")
        # Resist memory attacks
        _connection.execute("PRAGMA cipher_memory_security = ON")
        ...
        try:
            ...
            if "file is not a database" in str(e):
                raise ValueError(
                    "Wrong TASK_DB_ENCRYPTION_KEY — database cannot be opened")
...
def delete_task_cache(telegram_id: str, task_id: str) -> bool:
    ...
    try:
        with get_db_connection() as conn:
            conn.execute(
                "DELETE FROM tasks WHERE task_id = ? AND telegram_id = ?", (task_id, telegram_id))
            conn.commit()
        return True
    except:
        return False
