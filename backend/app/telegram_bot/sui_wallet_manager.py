...
if not SERVER_MASTER_KEY:
    raise EnvironmentError(
        "SERVER_MASTER_KEY is required in .env for double encryption!")
...
class SuiWalletManager:
    ...
    def create_wallet_for_user(self, user_id: str, password: str, word_count: int = 12) -> Dict[str, Any]:
        ...
        try:
            ...
            encrypted_keystring = self._encrypt_data(
                keystring.encode(), password)
            ...
            encrypted_wallet_path = self.storage_path / \
                f"{user_id}_wallet.json.enc"
            encrypted_payload = fernet.encrypt(
                json.dumps(wallet_data, indent=2).encode())
    ...
    def get_wallet(self, user_id: str, password: str) -> Optional[Any]:
        ...
        try:
            ...
            keystring = self._decrypt_data(
                wallet_data['encrypted_keystring'], password).decode()
    ...
    def get_address_from_keypair(self, user_id: str, password: str) -> Optional[str]:
        ...
        try:
            ...
            mnemonic = self._decrypt_data(
                wallet_data['encrypted_mnemonic'], password).decode()
