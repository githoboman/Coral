...
export interface ChainAddresses {
   ...
   * `null` disables it (account created with no attesters; SmartSessions
   * `useRegistry: false`). Disabled on Base Sepolia because, as verified on
   * 2026-08-23, neither the Rhinestone attester nor its mock attester has
   * attested SmartSessions or the V2 policies there — with gating on, every
   * install reverts with InsufficientAttestations. FR-1.5 remains enforced by
   * the pinned codehashes above and the module monitor. MAINNET GATE: re-run
   * the attestation check and enable gating before pinning mainnet addresses.
   */
  readonly registryGating: { readonly attesters: readonly Address[]; readonly threshold: number } | null;
}
...
export const BASE_SEPOLIA: ChainAddresses = Object.freeze({
  ...
  // same mechanism mainnet uses with Rhinestone's attester. See
  // ChainAddresses.registryGating for why Rhinestone's attester cannot be
  // used on Base Sepolia.
  registryGating: { attesters: ["0x91ac808850c33E15dc028a12Bfcaad70F1F8e6f9" as Address], threshold: 1 },
});
