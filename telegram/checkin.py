...
class CheckInManager:
    ...
    async def has_checked_in_recently(self, user_id: str, context: ContextTypes.DEFAULT_TYPE):
        ...
        if password:
            try:
                ...
                if last_ts:
                    last_checkin = datetime.fromtimestamp(last_ts / 1000.0, tz=pytz.UTC)
    ...
    async def record_check_in(self, user_id: str, context: ContextTypes.DEFAULT_TYPE) -> bool:
        try:
            ...
            current_timestamp_ms = int(datetime.now(pytz.UTC).timestamp() * 1000)
            ...
            if public_key and profile_id:
                blob_id = self.walrus.store_encrypted_user_data(public_key, checkin_data)
                if blob_id:
    ...
    async def process_check_in(self, user_id: str, context: ContextTypes.DEFAULT_TYPE) -> Dict[str, Any]:
        try:
            ...
            if current_streak in [5, 10, 15, 20, 25, 30]:
                ...
                next_milestone = min((d for d in [5, 10, 15, 20, 25, 30] if d > current_streak), default=None)
                next_reward = get_streak_reward_points(next_milestone) if next_milestone else 1
                msg = (
...
checkin_status_handler = CommandHandler('checkin_status', checkin_status_command)
