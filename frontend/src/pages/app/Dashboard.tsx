...
import { Send, Trash2, Plus, MoreVertical } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const fetchChatHistory = async () => {
      try {
        ...
        setMessages(prev => [
          ...
          {
            id: prev.length + 1,
            text: 'Error fetching chat history.',
            sender: 'ai',
    ...
    try {
      ...
      if (response.ok) {
        ...
        const aiMessage: Message = {
          ...
          text: data.response || 'I got your query!',
          sender: 'ai',
          timestamp: new Date().toLocaleTimeString(),
          chat_id: newChatId,
        };
  ...
  const handleDeleteChat = async (chatId: string) => {
    try {
      const response = await fetch(`${apiBaseUrl}/api/chat/${chatId}`, { method: 'DELETE' });
      if (response.ok) {
        const chatsResponse = await fetch(`${apiBaseUrl}/api/chats?user_id=${user_id}`);
        const chatsData = await chatsResponse.json();
        setChats(chatsData.chats);
          ...
          <AnimatePresence>
                ...
                <div
                  className={`max-w-[75%] rounded-2xl p-4 ${
                    msg.sender === 'user'
                      ...
                      : 'bg-white/5 text-white border border-white/10 shadow-sm backdrop-blur-sm'
                  }`}
                >
                  {msg.sender === 'ai' ? (
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        code: ({ children }) => (
                          <code className="bg-black/40 px-1 py-0.5 rounded text-emerald-300">{children}</code>
                        ),
                      }}
                    >
                      {msg.text}
            ...
            {isLoading && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start mb-4">
                <div className="bg-white/10 p-3 rounded-xl max-w-[70%] flex space-x-2">
      ...
      <div className={`w-full p-4 ${messages.length > 0 ? 'absolute bottom-0' : ''}`}>
        ...
        >
          <textarea
            ...
            className="w-full bg-transparent placeholder-white/40 border-0 focus:outline-none resize-none min-h-[3rem] max-h-[120px] overflow-y-auto p-2"
            maxLength={500}
            disabled={isLoading}
          />
          {input.trim() && (
            <button
              ...
              className={`rounded-full w-9 h-9 flex items-center justify-center bg-gradient-to-r from-[#8EF1FE] to-[#0796D9] text-black transition-all duration-200 ${
                isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:from-[#79e8f0] hover:to-[#0687c2]'
              }`}
            >
              <Send size={16} />
            </button>
