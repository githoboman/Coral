...
| **D25** | **Account implementation (T-007, 22 Aug 2026)** | **Safe (singleton + proxy factory) behind Rhinestone's Safe7579 adapter**, created via permissionless.js `toSafeSmartAccount` with ERC-7579 config; counterfactual address derived from owner + salt (FR-1.1). Module addresses pinned in `server/src/services/evm/addresses.ts` with CI codehash assertions — never resolved from a registry at runtime | Stakeholder decision. Safe core is the most audited smart-account code in production; Safe7579 is Rhinestone's primary SmartSessions target; keeps PRD Q9 (users' existing Safes) open. Trade: slightly higher gas per op than Kernel/Nexus — accepted |
| **D26** | **Rolling 24h execution cap (FR-2.7), 23 Aug 2026** | **Both layers:** a second custom contract, `CorralRateLimitPolicy.sol` — a SmartSessions userOp policy enforcing ≤N executions per rolling 24h window per session on-chain — **plus** the off-chain scheduler enforcing the same pacing (defense in depth, FR-6 pattern). Composed into sessions like any other policy; verified by post-install read-back like any other policy | Stakeholder decision: Rhinestone's V2 policy set has only a lifetime UsageLimit. Audit scope grows by one small contract. **Caveat recorded:** a time-based check in the ERC-4337 validation phase uses `TIMESTAMP`, which public bundlers' mempool rules forbid — acceptable only because D13 (own relayer, no third-party bundler) is permanent. Supersedes the "custom Solidity = CorralJournal only" line in CLAUDE.md §3 |
