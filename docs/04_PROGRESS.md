...
| | |
|---|---|
| **Current phase** | **Backend complete through the execution engine.** Next epic: FE re-point (Sui → Base) |
| **Active ticket** | — |
| **Done this epic** | Uniswap v3 adapter ✓, deterministic compiler ✓, preflight ✓, session signer + agent userOp ✓ (**J2 live**), persistence + queue + ledger ✓, reconciliation ✓, off-chain kill switch ✓, **chaos: zero double-executions across 5 kill points ✓ (G2)** |
| **Next up** | FE: policy review (FR-11.1), budget meters (FR-11.3), in-app kill switch (FR-11.4), activity feed (FR-11.6); then notifications, KMS custody (D23), audit |
...
| **Blocked / waiting** | 1. **Supabase `DATABASE_URL`** — the supplied value is a bare hostname that does not resolve; need the full session-pooler URI with password (session 13). 2. `github.com/olaDmenace/corral` not created. 3. Alchemy key rotation (leaked in a transcript, session 8) |
