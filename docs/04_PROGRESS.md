...
| **Active ticket** | Uniswap v3 adapter (`ProtocolAdapter`, old C-401/402) → deterministic `Plan → calldata` compiler → preflight gate → pipeline with idempotency ledger |
| **Next up** | Chaos harness (zero double-executions), then FE re-point (Sui → Base) |
