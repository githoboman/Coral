...
| | |
|---|---|
| **Current phase** | **Bootstrap + violation matrix complete.** Next epic: adapter + planner + execution pipeline |
| **Active ticket** | Execution pipeline persistence: jobs queue + idempotency ledger + scheduler + budget mirror (old C-501…C-506) — **blocked on Supabase credentials** (`DATABASE_URL`, `SUPABASE_URL`, service key) |
| **Done this epic** | Uniswap v3 adapter ✓, deterministic compiler ✓, preflight ✓, session signer + agent userOp ✓ — **J2 proven live on Base Sepolia** (salt-8 account) |
| **Next up** | Chaos harness (zero double-executions, G2), then FE re-point (Sui → Base) |
