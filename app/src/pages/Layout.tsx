...
const MobileSidebarDrawer = ({ isOpen, onClose, isDashboard, navItems, signOut }: { isOpen: boolean, onClose: () => void, isDashboard: boolean, navItems: any[], signOut: () => void }) => {
  ...
  useGSAP(() => {
    if (isOpen && backdropRef.current && containerRef.current) {
      ...
      gsap.to(containerRef.current, { x: 0, duration: 0.4, ease: 'power3.out' });
    } else if (!isOpen && backdropRef.current && containerRef.current) {
      gsap.to(backdropRef.current, { opacity: 0, duration: 0.3, ease: 'power2.in' });
      gsap.to(containerRef.current, { x: '-100%', duration: 0.4, ease: 'power3.in', onComplete: () => setShouldRender(false) });
    }
  }, [isOpen, shouldRender]);
  ...
  return (
    <>
      <div ref={backdropRef} className="fixed inset-0 bg-black/60 z-[70] md:hidden backdrop-blur-sm opacity-0 cursor-pointer" onClick={onClose} />
      <div ref={containerRef} className="fixed inset-y-0 left-0 w-64 bg-black z-[80] p-0 border-r border-white/10 md:hidden overflow-y-auto -translate-x-full">
        {isDashboard ? (
...
const WalletManager = ({
  isWalletCollapsed, toggleWallet, walletBalanceUSD, isBalanceVisible, setIsBalanceVisible,
  activeWalletModal, setActiveWalletModal, isSettingsOpen, toggleSettings,
  showConfirmation, setShowConfirmation, sendAmount, setSendAmount, selectedSendToken,
  ...
  useGSAP(() => {
    if (!isWalletCollapsed && containerRef.current) {
      gsap.fromTo(containerRef.current, { x: '100%', opacity: 0 }, { x: 0, opacity: 1, duration: 0.5, ease: 'expo.out' });
    } else if (isWalletCollapsed && containerRef.current) {
      gsap.to(containerRef.current, { x: '100%', opacity: 0, duration: 0.4, ease: 'expo.in', onComplete: () => setShouldRender(false) });
    }
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center text-center w-full mb-8">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setIsBalanceVisible(!isBalanceVisible)} className="text-white/20 hover:text-white/40 transition-colors mt-2 cursor-pointer">
        ...
        <div className="flex-1 w-full overflow-y-auto pr-2 scrollbar-none">
          ...
          {activeTab === 'Tokens' && (
            <div className="space-y-4">
              {tokens.length > 0 ? tokens.map((token: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between group cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center p-2.5 relative" style={{ background: token.icon.startsWith('/') ? 'transparent' : 'linear-gradient(135deg, #00FF88 0%, #0061FF 100%)' }}>
                      {token.icon.startsWith('/') ? <img src={token.icon} alt={token.symbol} className="w-full h-full object-contain" /> : <span className="text-white font-bold text-xs">{token.icon}</span>}
                    </div>
                    <div>
                      <div className="text-white font-bold text-sm">{token.symbol}</div>
                      <div className="text-[10px] text-white/40 font-medium">
                        {isBalanceVisible ? `${token.balance.toFixed(4)} ${token.symbol}` : `•••• ${token.symbol}`}
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-white font-bold text-sm">
                      {isBalanceVisible ? `$${(token.balance * token.price).toFixed(2)}` : '••••'}
                    </div>
                    <div className={`text-[10px] font-bold ${token.change24h >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                      {token.change24h >= 0 ? '+' : ''}{token.change24h.toFixed(2)}%
                    </div>
                  </div>
                </div>
              )) : (
                <div className="text-center py-10 text-white/20 text-xs font-medium">No tokens found</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'Collectibles' && (
            <div className="grid grid-cols-2 gap-3">
              {nfts.length > 0 ? nfts.map((nft: any, idx: number) => (
                <div key={idx} className="bg-white/5 rounded-2xl overflow-hidden aspect-square relative group cursor-pointer">
                  {nft.image ? (
                ...
                <div className="col-span-2 text-center py-10 text-white/20 text-xs font-medium">No collectibles found</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'Activity' && (
            <div className="space-y-4">
              {activity.length > 0 ? activity.map((tx: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between group cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center">
                      <ArrowUp size={16} className={`text-white/40 ${tx.effects?.status?.status === 'success' ? 'text-emerald-400' : 'text-red-400'}`} />
                    </div>
                    <div>
                      <div className="text-white font-bold text-xs">Transaction</div>
                      <div className="text-[10px] text-white/40 font-medium">{tx.digest.slice(0, 10)}...</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-white font-bold text-xs">{tx.effects?.status?.status === 'success' ? 'Success' : 'Failed'}</div>
                    <div className="text-[10px] text-white/40 font-medium">{new Date(Number(tx.timestampMs)).toLocaleDateString()}</div>
                  </div>
                </div>
              )) : (
                <div className="text-center py-10 text-white/20 text-xs font-medium">No recent activity</div>
              )}
            </div>
          )}
        </div>
...
const WalletModalOverlay = (props: any) => {
  ...
  useGSAP(() => {
    if (activeWalletModal || isSettingsOpen) {
      ...
      gsap.fromTo(overlayRef.current, { x: '100%' }, { x: 0, duration: 0.4, ease: 'power3.out' });
    } else {
      gsap.to(overlayRef.current, { x: '100%', duration: 0.4, ease: 'power3.in', onComplete: () => setShouldRender(false) });
    }
  ...
  return (
    <div ref={overlayRef} className="absolute inset-0 bg-[#070B0F] z-50 flex flex-col p-6">
      <div className="flex justify-between items-center w-full mb-8">
        <button
          onClick={() => {
            if (showConfirmation) setShowConfirmation(false);
            else { setActiveWalletModal(null); if (isSettingsOpen) toggleSettings(); }
          }}
        ...
        <span className="font-bold text-white text-base">
          {isSettingsOpen ? 'Settings' : activeWalletModal ? activeWalletModal.charAt(0).toUpperCase() + activeWalletModal.slice(1) : ''}
        </span>
        <div className="w-8" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none">
          ...
          <div className="space-y-4">
            <div className="bg-white/[0.03] border border-white/5 rounded-3xl p-5 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[11px] text-white/40">Balance: {selectedSendToken?.balance?.toFixed(4) || '0.0000'}</span>
              </div>
              <div className="flex justify-between items-end">
                <div className="flex flex-col">
                  ...
                  />
                  <span className="text-sm text-white/20 font-medium">≈ ${(Number(sendAmount || 0) * (selectedSendToken?.price || 0)).toFixed(2)}</span>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <button onClick={() => setSendAmount(selectedSendToken?.balance?.toString() || '0')} className="text-[10px] font-bold text-[#82E131] hover:underline mb-2 cursor-pointer">MAX</button>
                  <button className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10 hover:bg-white/10 transition-colors cursor-pointer">
                    <div className="w-4 h-4 rounded-full overflow-hidden flex-shrink-0">
                      <img src={selectedSendToken?.icon || '/assets/images/sui-icon.png'} className="w-full h-full object-contain" />
                    </div>
                    <span className="text-xs font-bold text-white">{selectedSendToken?.symbol || 'SUI'}</span>
          ...
          <div className="space-y-2">
            <div className="bg-white/[0.03] border border-white/5 rounded-3xl p-5 space-y-4">
              <div className="flex justify-between items-center text-sm font-bold text-white">
                <span>You pay</span>
                <span className="text-[11px] text-white/40">Balance: {swapFromToken?.balance?.toFixed(4) || '0.0000'}</span>
              </div>
              <div className="flex justify-between items-end">
                <input type="text" value={swapFromAmount} onChange={(e) => setSwapFromAmount(e.target.value)} placeholder="0" className="text-[32px] font-bold text-white leading-tight bg-transparent focus:outline-none w-2/3" />
                <button className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10 cursor-pointer">
                  <div className="w-4 h-4 rounded-full overflow-hidden flex-shrink-0">
                    <img src={swapFromToken?.icon || '/assets/images/sui-icon.png'} className="w-full h-full object-contain" />
                  </div>
                  <span className="text-xs font-bold text-white">{swapFromToken?.symbol || 'SUI'}</span>
                  <ChevronDown size={14} className="text-white/40" />
                </button>
              </div>
            </div>
            ...
            <div className="p-[1px] bg-gradient-to-r from-[#B7FC0D] to-[#246AFC] rounded-3xl">
              <div className="bg-[#070B0F] rounded-[inherit] p-5 space-y-4">
                <div className="flex justify-between items-center text-sm font-bold text-white">
                  ...
                  <span className="text-[11px] text-white/40">Balance: {swapToToken?.balance?.toFixed(4) || '0.0000'}</span>
                </div>
                <div className="flex justify-between items-end">
                  <span className="text-[32px] font-bold text-white">{swapToAmount || '0'}</span>
                  <button className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10 cursor-pointer">
                    <div className="w-4 h-4 rounded-full overflow-hidden flex-shrink-0">
                      <img src={swapToToken?.icon || '/assets/images/usdc-icon.png'} className="w-full h-full object-contain" />
                    </div>
                    <span className="text-xs font-bold text-white">{swapToToken?.symbol || 'USDC'}</span>
                    <ChevronDown size={14} className="text-white/40" />
                  </button>
                </div>
              </div>
            ...
            <div className="flex justify-between items-center px-1">
              <span className="text-[10px] font-bold text-white/40">1 SUI = ${swapRate.toFixed(2)}</span>
          ...
          <div className="flex flex-col items-center gap-6 pt-4">
            <div className="p-[1px] bg-gradient-to-r from-[#B7FC0D] to-[#246AFC] rounded-2xl w-full">
              <div className="w-full relative group bg-[#070B0F] rounded-[inherit]">
                ...
                <button onClick={() => copyToClipboard(address || '')} className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/5 rounded-xl flex items-center justify-center hover:bg-white/10 transition-colors border border-white/10 cursor-pointer">
                  <Copy size={14} className="text-white/40" />
                </button>
              </div>
            </div>
            <div className="w-48 h-48 bg-white rounded-2xl p-4 flex items-center justify-center shadow-xl">
              <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${address}`} alt="Wallet QR" className="w-full h-full" />
            </div>

            <div className="flex items-center justify-between w-full px-1">
              <div className="flex flex-col">
                <span className="text-[13px] font-bold text-white">Share QR code instead</span>
                <span className="text-[9px] font-bold text-white/20">Send ONLY sui tokens to this address. <span className="text-[#82E131] cursor-pointer">Learn more</span></span>
              </div>
              <button
        ...
        {isSettingsOpen && !showConfirmation && (
          <div className="space-y-6">
            <div className="space-y-2">
              <h3 className="text-[11px] font-bold text-white/20 uppercase px-1">Security details</h3>
              {[
                { icon: <Key size={18} />, label: 'View SUI private key' },
                { icon: <ShieldCheck size={18} />, label: 'View passkeys' },
              ].map((item, idx) => (
                <div key={idx} className="flex items-center justify-between p-4 bg-white/[0.03] border border-white/5 rounded-2xl cursor-pointer hover:bg-white/10 transition-all group">
                  <div className="flex items-center gap-3">
        ...
        <div className="mt-8 space-y-3">
          {!showConfirmation ? (
            ...
            <div className="flex flex-row gap-3">
              ...
              >
                {isSettingsOpen ? 'Confirm logout?' : `Confirm ${activeWalletModal}?`}
              </button>
              <button onClick={() => setShowConfirmation(false)} className="flex-1 h-14 bg-[#FF5252] text-white font-bold text-[15px] rounded-3xl cursor-pointer transition-all active:scale-[0.98]">
...
export default function AppLayout() {
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      ...
      let totalUsd = 0;
      const KNOWN_TOKENS = {
        '0x2::sui::SUI': { symbol: 'SUI', decimals: 9, price: suiData.price, change24h: suiData.change, icon: '/assets/images/sui-icon.png' },
        '0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN': { symbol: 'USDC', decimals: 6, price: 1.0, change24h: 0, icon: '/assets/images/usdc-icon.png' }
      };

      // Always show SUI and USDC
      const displayTokens: Record<string, any> = {
        '0x2::sui::SUI': { ...KNOWN_TOKENS['0x2::sui::SUI'], balance: 0, value: 0, type: '0x2::sui::SUI' },
        '0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN': { ...KNOWN_TOKENS['0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN'], balance: 0, value: 0, type: '0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN' }
      };
  ...
  }, [address, lastFetched, suiClient, fetchSuiPriceUSD, selectedSendToken, swapFromToken, swapToToken]);
