    ...
    <div className="relative min-h-screen w-full bg-[#070B0F] text-white overflow-x-hidden">
      ...
      <header className="relative z-20 mx-auto max-w-6xl flex items-center justify-between px-6 py-5">
        <div className="flex items-center gap-2">
          <img src="/assets/coral-mark.svg" alt="Coral" className="w-10 h-10" />
          <span className="font-bold text-lg tracking-tight">Coral</span>
        </div>
        <div className="flex items-center gap-2 text-[12px] font-medium text-white/50">
          ...
          <button onClick={scrollToConnect} className="rounded-full bg-white text-[#070B0F] font-semibold px-4 py-2 hover:bg-white/90 transition-all cursor-pointer">
            Launch app
          </button>
        </div>
      </header>
      ...
      {/* ── Hero ───────────────────────────────────────────────────────── */}
      <section className="relative z-10 mx-auto max-w-6xl px-6 pt-14 pb-20 grid lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center">
        <div className="coral-fade-up coral-d1">
          <span className="inline-flex items-center gap-2 rounded-full border border-[#FF6A4D]/30 bg-[#FF6A4D]/10 px-3 py-1.5 text-[12px] font-semibold text-[#FF9472] mb-6">
            <FiCpu className="text-[13px]" /> Agentic Web · Autonomous on-chain trading
          </span>
          <h1 className="text-[44px] sm:text-[58px] font-bold leading-[1.05] tracking-tight">
          ...
          <p className="mt-6 text-[17px] leading-relaxed text-white/60 max-w-[560px]">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <button onClick={scrollToConnect} className="group inline-flex items-center gap-2 rounded-full bg-[#FF6A4D] hover:bg-[#FF7A5E] text-white font-semibold px-6 py-3.5 transition-all hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(255,106,77,0.35)] cursor-pointer">
              Connect & delegate
              <FiArrowRight className="transition-transform group-hover:translate-x-0.5" />
            </button>
            <a href="#how" className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-6 py-3.5 font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">
              See how it works
            </a>
          </div>
        ...
        {/* Connect card */}
        <div id="connect" className="coral-fade-up coral-d3 w-full">
          <div className="rounded-[28px] border border-white/10 bg-[#0D1117]/70 backdrop-blur-xl p-7 shadow-[0_30px_80px_-20px_rgba(255,106,77,0.25),0_20px_50px_rgba(0,0,0,0.5)]">
      ...
      {/* ── How it works ───────────────────────────────────────────────── */}
      <section id="how" className="relative z-10 mx-auto max-w-6xl px-6 py-16 border-t border-white/5">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {[
            ...
            <div key={s.n} className="group relative rounded-3xl border border-white/10 bg-white/[0.03] p-7 hover:border-[#FF6A4D]/30 hover:bg-white/[0.05] transition-all">
      ...
      {/* ── Policy guide ───────────────────────────────────────────────── */}
      <section className="relative z-10 mx-auto max-w-6xl px-6 py-16 border-t border-white/5">
        <SectionHeader
          eyebrow="The policy"
...
function SectionHeader({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle?: string }) {
  return (
    <div className="mb-9 max-w-2xl">
      <div className="text-[12px] font-bold uppercase tracking-[0.18em] text-[#FF9472] mb-3">{eyebrow}</div>
      <h2 className="text-[30px] sm:text-[36px] font-bold tracking-tight leading-tight">{title}</h2>
      {subtitle && <p className="mt-3 text-[15px] leading-relaxed text-white/55">{subtitle}</p>}
    </div>
  );
}
