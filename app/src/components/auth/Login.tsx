...
export function LoginModal({ isOpen, loading }: Omit<LoginModalProps, 'onSignIn' | 'onSignInWithGoogle'>) {
        ...
        <div className="relative z-0 p-6 sm:p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-white/60 text-sm sm:text-base leading-relaxed max-w-sm mx-auto">
              Securely access your Tovira account using Google.
            </p>
          </div>
...
export function LoginDrawer({ isOpen, loading }: Omit<LoginDrawerProps, 'onSignIn' | 'onSignInWithGoogle'>) {
  const { mutate: connect } = useConnectWallet();
  const wallets = useWallets().filter(isEnokiWallet);
  const [isConnecting, setIsConnecting] = useState(false);
