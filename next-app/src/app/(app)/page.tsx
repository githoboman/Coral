              ...
              <button
                onClick={() => {
                  if (typeof window !== 'undefined' && window.navigator.clipboard) {
                    window.navigator.clipboard.writeText(String(children));
                  }
                }}
                className="p-1.5 hover:bg-white/10 rounded-md transition-colors text-gray-400 hover:text-white cursor-pointer"
                title="Copy code"
...
const Dashboard = () => {
    ...
    const executePendingTransaction = async () => {
      try {
        ...
        const successMessage: Message = {
          ...
          text: `**Transaction Successful!**\n\n${description} completed.\n\n[View on Explorer](https://suiscan.xyz/${process.env.NEXT_PUBLIC_SUI_NETWORK || 'testnet'}/tx/${result.digest})`,
          sender: 'ai',
  ...
  useEffect(() => {
    if (agentParam) {
      ...
      if (mappedId !== selectedAgentId) {
        ...
        router.push('/?agent=' + agentParam);
      }
    }
  }, [agentParam, selectedAgentId, dispatch, router]);
  ...
  // Mobile actions registration removed as Layout context handling is different in Next.js
  useEffect(() => {
    // Mobile actions can be handled via a custom hook or provider if needed
    return () => { };
  }, [startNewChat]);
