...
export default function AccountPage() {
  ...
  const fetchAccountData = useCallback(async () => {
    ...
    try {
      const [accountRes, claimRes] = await Promise.all([
        fetch(`/api/account?user_id=${encodeURIComponent(addr)}`),
        fetch(`/api/auth/check-claim-status?wallet_address=${encodeURIComponent(addr)}`),
      ]);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-6 animate-in fade-in duration-500">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold">My Account</h1>
      </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-white/5 text-white/80">
                  {leaderboard.slice(0, 100).map((entry) => (
                    <tr
                      key={entry.user_id}
                      className={`hover:bg-white/5 transition-colors ${entry.user_id === currentAccount?.address
                        ? "bg-teal-500/10"
                        : ""
                        }`}
                    >
