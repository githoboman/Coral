...
async function getCheckInStatus(userId: string): Promise<CheckInStatus> {
...
async function processCheckIn(userId: string): Promise<CheckInResult> {
  const supabase = getSupabaseClient();
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[API] Error getting check-in status:', error);
    return NextResponse.json(
...
/**
 * POST /api/checkin - Process daily check-in
 */
